		...
		<div id="eq-search-overlay">
			<img id="eq-search-overlay-img" src="<?php echo EQUIPEER_URL; ?>assets/images/loader-ajax-search.gif" alt="Search">
		</div>
		...
		<script>
			jQuery(function($) {
				...
				// ===== TYPE D'ANNONCES =====	
				// ===========================
				$('#type_annonce_expert, #type_annonce_libre').click(function() {
					// Ajax search
					eq_ajax_search();
				});
				// -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
			...
			function eq_ajax_search() {
				// Debug
				var ajax_debug = false;
				// Animation start
				eqon();
				var search_form = jQuery('#searchMainForm').serialize();
				if (ajax_debug) console.log('Serialize: '+search_form);
				jQuery.ajax({
					type: "GET",                // use $_POST request to submit data
					url: equipeer_ajax.ajaxurl,  // URL to "wp-admin/admin-ajax.php"
					data: search_form + '&action=' + 'equipeer_ajax_search', // wp_ajax_*, wp_ajax_nopriv_*
					success: function(response) {
						jQuery('#search-validation span#search_counter').html('&nbsp;(' + response + ')');
						if (ajax_debug) console.log('Search (Response: '+response+'): '+search);
						eqoff(); // Animation end
					},
			...
			function eqon() {
				document.getElementById("eq-search-overlay").style.display = "block";
				document.getElementById("eq-search-overlay-img").style.display = "block";
			}
			function eqoff() {
				document.getElementById("eq-search-overlay").style.display = "none";
				document.getElementById("eq-search-overlay-img").style.display = "none";
			}
		</script>
		<style>
			#eq-search-overlay {
