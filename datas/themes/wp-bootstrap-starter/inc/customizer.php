...
function wp_bootstrap_starter_customize_register( $wp_customize ) {
    ...
    // =================== HEADER TOPBAR ====================
    // ======================================================
    // ===== Topbar Social networks / Phone =====
    $id_topbar = 'topbar';
    $wp_customize->add_section(
        $id_topbar,
        array(
            'title' => __( 'Topbar', 'wp-bootstrap-starter' ),
            'description' => "<span class='description'><p>Si vous indiquez un lien sur un de vos réseaux sociaux, celui-ci s'affichera sous forme d'icône dans la TOPBAR et dans le pied de page.</p><p>Indiquez le numéro de téléphone (texte) ainsi que le lien qui sera associé à ce texte qui permettra aux smartphones de pouvoir directement accéder au numéro de téléphone.<br>Les deux informations sont nécessaires pour afficher le numéro de téléphone.</p></span>",
            'priority' => 25,
        )
    );
    // ===== FACEBOOK =====
    ...
    $wp_customize->add_control( new WP_Customize_Control($wp_customize, 'phone_text', array(
        ...
        'section'     => $id_topbar,
        'settings'    => 'phone_text',
        'description' => 'Ex: +33(0)6 33 85 72 90',
        'type'        => 'text'
    ) ) );
    // ===== TELEPHONE (Link) =====
    ...
    $wp_customize->add_control( new WP_Customize_Control($wp_customize, 'phone_link', array(
        'label'       => 'Téléphone (Lien)',
        'section'     => $id_topbar,
        'settings'    => 'phone_link',
        'description' => 'Ex: +33633857290',
        'type'        => 'text'
    ) ) );
    ...
    $wp_customize->add_section(
        $id_navbar,
        array(
            'title' => __( 'Navbar', 'wp-bootstrap-starter' ),
            'description' => "",
            'priority' => 27,
        )
    );
    // ===== TEXT UNDER LOGO =====
    $wp_customize->add_setting( 'text_under_logo', array(
        'default' => 'Vente et achat de chevaux et poneys de sport expertisés',
        'sanitize_callback' => 'wp_filter_nohtml_kses',
    ) );
    $wp_customize->add_control( new WP_Customize_Control($wp_customize, 'text_under_logo', array(
        'label'       => 'Texte sous logo',
        'section'     => $id_navbar,
        'settings'    => 'text_under_logo',
        'description' => 'Ex: Vente et achat de chevaux et poneys de sport expertisés',
        'type'        => 'text'
    ) ) );
    $wp_customize->add_setting( 'text_under_logo_en', array(
        'default' => 'Sale and purchase of appraised sport horses and ponies ',
        'sanitize_callback' => 'wp_filter_nohtml_kses',
    ) );
    $wp_customize->add_control( new WP_Customize_Control($wp_customize, 'text_under_logo_en', array(
        'label'       => 'Texte sous logo (EN)',
        'section'     => $id_navbar,
        'settings'    => 'text_under_logo_en',
        'description' => 'Ex: Vente et achat de chevaux et poneys de sport expertisés',
        'type'        => 'text'
    ) ) );
    ...
    // ===== One / Two button search banner =====
    $wp_customize->add_setting( 'home_search_one_button', array(
        'capability'        => 'edit_theme_options',
        'sanitize_callback' => 'themeslug_sanitize_checkbox',
    ) );
    $wp_customize->add_control( new WP_Customize_Control($wp_customize, 'home_search_one_button', array(
        'settings'    => 'home_search_one_button',
        'label'       => __('Two buttons in search Banner', 'wp-bootstrap-starter'),
        'section'     => 'header_image',
        'description' => 'Un bouton par défaut (Voir toutes les annonces).<br>Si vous cochez, il y aura un deuxième bouton (Déposer votre annonce).',
        'type'        => 'checkbox',
    ) ) );
