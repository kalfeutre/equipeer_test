...
$get_bloginfo_description = trim( get_bloginfo( 'description' ) );
...
<?php
	if (get_post_meta( $the_id, 'impression', true ) != '') {
		echo nl2br( get_post_meta( $the_id, 'impression', true ) );
	} else {
...
if ( !is_user_logged_in() ) {
	...
	$detailed_description = [
		...
		,__( 'Skills & qualities', 'wp-bootstrap-starter' ) => nl2br($skills_qualities_1 . $skills_qualities_2 . $skills_qualities_3)
		...
		,__( 'Rider profile', 'wp-bootstrap-starter' )      => @get_post_meta( $the_id, 'cavalier_profil', true )
